...
//返回最新文档
func (m *Document) FindToPager(pageIndex, pageSize int) (docs []*Document, totalCount int, err error) {
	o := orm.NewOrm()
	...
	_, err = o.QueryTable(m.TableNameWithPrefix()).OrderBy("-create_time").Offset(offset).Limit(pageSize).All(&docs)

	return
}

func (m *Document) FindToPagerByUserId(pageIndex, pageSize, id int) (docs []*Document, totalCount int, err error) {
	o := orm.NewOrm()
	count, err := o.QueryTable(m.TableNameWithPrefix()).Filter("member_id", id).Count()
	...
	totalCount = int(count)
	offset := (pageIndex - 1) * pageSize
	_, err = o.QueryTable(m.TableNameWithPrefix()).Filter("member_id", id).OrderBy("-create_time").Offset(offset).Limit(pageSize).All(&docs)

	return
}

func (m *Document) FindToPagerByBookId(pageIndex, pageSize, id int) (docs []*Document, totalCount int, err error) {
	o := orm.NewOrm()
	count, err := o.QueryTable(m.TableNameWithPrefix()).Filter("member_id", id).Count()
	...
	totalCount = int(count)
	offset := (pageIndex - 1) * pageSize
	_, err = o.QueryTable(m.TableNameWithPrefix()).Filter("book_id", id).OrderBy("-create_time").Offset(offset).Limit(pageSize).All(&docs)

	return
}

//上一篇文档
func (m *Document) FindPrev(book_id int) (*Document, error) {
	if book_id <= 0 {
		return m, ErrInvalidParameter
	}
	o := orm.NewOrm()

	err := o.QueryTable(m.TableNameWithPrefix()).Filter("document_id__lt", book_id).OrderBy("-create_time").Limit(1).One(m)
...
//上一篇文档
func (m *Document) FindNext(book_id int) (*Document, error) {
	if book_id <= 0 {
		return m, ErrInvalidParameter
	}
	o := orm.NewOrm()

	err := o.QueryTable(m.TableNameWithPrefix()).Filter("document_id__gt", book_id).OrderBy("create_time").Limit(1).One(m)
